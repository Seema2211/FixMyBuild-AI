...
<div class="auth-shell">
  <div class="auth-card">
    <div class="auth-logo">
      <svg width="36" height="36" viewBox="0 0 30 30" fill="none">
        <rect width="30" height="30" rx="8" fill="url(#lg2)"/>
        <path d="M9 21l6-12 6 12M11.5 17h7" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
        <defs><linearGradient id="lg2" x1="0" y1="0" x2="30" y2="30" gradientUnits="userSpaceOnUse">
    ...
    @if (invitePreview) {
      <div class="invite-banner">
        ...
        You've been invited to join <strong>{{ invitePreview.orgName }}</strong> as <strong>{{ invitePreview.role }}</strong>.
      </div>
    }
    <h1 class="auth-title">{{ invitePreview ? 'Accept invitation' : 'Create your workspace' }}</h1>
    <p class="auth-subtitle">{{ invitePreview ? 'Create your account to join the team.' : 'Start monitoring your pipelines in minutes' }}</p>

    <form class="auth-form" (ngSubmit)="submit()">
      <div class="field-row">
        <div class="field-group">
          <label class="field-label">First name</label>
          <input type="text" class="field-input" placeholder="Jane"
            [(ngModel)]="form.firstName" name="firstName" required autocomplete="given-name">
        </div>
        <div class="field-group">
          <label class="field-label">Last name</label>
          <input type="text" class="field-input" placeholder="Smith"
            [(ngModel)]="form.lastName" name="lastName" required autocomplete="family-name">
        </div>
      </div>
      ...
      <div class="field-group">
        ...
        <input type="email" class="field-input" placeholder="jane@company.com"
          [(ngModel)]="form.email" name="email" required autocomplete="email">
      </div>

      @if (!inviteToken) {
        <div class="field-group">
          <label class="field-label">Organization name</label>
          <input type="text" class="field-input" placeholder="Acme Corp"
            [(ngModel)]="form.organizationName" name="orgName">
          <span class="field-hint">Your team or company name. You can invite members later.</span>
        </div>
      }
  ...
  styles: [`
    ...
    .auth-title { font-size: 1.625rem; font-weight: 800; color: #0f172a; letter-spacing: -0.03em; margin: 0 0 0.375rem; }
    ...
    .field-hint { font-size: 0.6875rem; color: #9ca3af; }
...
export class RegisterComponent implements OnInit {
  form = { email: '', password: '', firstName: '', lastName: '', organizationName: '' };
  showPassword = false;
  loading = false;
  inviteToken = '';
  invitePreview: { orgName: string; role: string } | null = null;
  ...
  ngOnInit(): void {
    ...
    if (this.inviteToken) {
      this.http.get<{ orgName: string; role: string; email: string }>(
        `${environment.apiUrl}/api/invitations/preview?token=${this.inviteToken}`
      ).subscribe({
        next: (preview) => {
          this.invitePreview = preview;
          if (preview.email) this.form.email = preview.email;
        },
        error: () => this.snackBar.open('Invite link is invalid or expired.', 'Dismiss', { duration: 5000 }),
      });
    }
  }
  ...
  submit(): void {
    ...
    if (!this.inviteToken && !this.form.organizationName) return;
    this.loading = true;

    if (this.inviteToken) {
      this.http.post<any>(`${environment.apiUrl}/api/auth/accept-invite`, {
        ...
        error: (err) => {
          this.loading = false;
          this.snackBar.open(err?.error?.message ?? 'Failed to accept invitation.', 'Dismiss', { duration: 5000 });
        },
      });
    } else {
