...
const STEPS = [
    ...
    visual: (
      <div className="flex flex-wrap gap-2 mt-4">
        {['GitHub', 'GitLab', 'Azure DevOps', 'Bitbucket'].map((p) => (
          <span key={p} className="px-2.5 py-1 text-xs font-medium bg-white/8 border border-white/12 rounded-lg text-slate-300">
            {p}
          </span>
        ))}
      </div>
  ...
  },
  {
    ...
    visual: (
      <div className="mt-4 rounded-xl bg-white/4 border border-white/8 p-3 font-mono text-xs space-y-1.5">
        <div className="text-red-400">✗ Step "test" failed after 43s</div>
        <div className="text-indigo-300">→ Root cause: Missing env var NODE_ENV</div>
  ...
  },
  {
    ...
    visual: (
      <div className="mt-4 flex items-center gap-3 rounded-xl bg-emerald-500/8 border border-emerald-500/20 px-3 py-2.5">
        ...
        <span className="ml-auto text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-medium">Open</span>
      </div>
    ),
  },
];
  ...
  return (
    <section id="how-it-works" className="py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="text-xs font-semibold uppercase tracking-widest text-indigo-400 mb-3">How it works</div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 relative">
          ...
          {STEPS.map((step, i) => (
            <motion.div
              ...
              className="relative bg-white/3 border border-white/8 rounded-2xl p-6 hover:border-indigo-500/30 hover:bg-white/5 transition-all duration-300 group"
            >
              {/* Step number + icon */}
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-900/40 group-hover:shadow-indigo-900/60 transition-shadow">
                  <step.icon className="w-5 h-5 text-white" />
                </div>
                <span className="text-2xl font-black text-white/10 group-hover:text-white/15 transition-colors">
                  {step.number}
                </span>
              </div>
